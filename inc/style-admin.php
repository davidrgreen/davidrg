 ...
 */
add_filter( 'admin_footer_text', 'davidrg_admin_footer' );
function davidrg_admin_footer() {
 ...
 */
add_action( 'wp_dashboard_setup', 'davidrg_remove_dashboard_widgets' );
function davidrg_remove_dashboard_widgets() {
 ...
 */
add_action('widgets_init', 'davidrg_remove_default_widgets', 11);
function davidrg_remove_default_widgets() {
 ...
 */
add_action( 'init', 'davidrg_maybe_disable_yoast_page_analysis' );
/**
 ...
 * @return  bool
 */
function davidrg_is_edit_screen( $post_types = '' ) {
 ...
 */
function davidrg_maybe_disable_yoast_page_analysis() {
	if ( ! davidrg_is_edit_screen() ) {
		return;
	}
...
add_action( 'load-customize.php', 'davidrg_customize' );

// Remove 'Customize' from Admin menu
function davidrg_remove_submenus() {
    global $submenu;
    // Appearance Menu
    unset($submenu['themes.php'][6]); // Customize
}
add_action('admin_menu', 'davidrg_remove_submenus');

// Remove 'Customize' from the Toolbar -front-end
function davidrg_remove_admin_bar_links() {
    global $wp_admin_bar;
    $wp_admin_bar->remove_menu('customize');
}
add_action( 'wp_before_admin_bar_render', 'davidrg_remove_admin_bar_links' );

// Add Custom CSS to Back-end head
function davidrg_admin_css() {
    echo '<style type="text/css">.theme-actions .button.customize { display:none; } </style>';
}
add_action('admin_head', 'davidrg_admin_css');
